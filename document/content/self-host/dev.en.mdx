...
description: Develop and debug gptGO locally
---

import { Alert } from '@/components/docs/Alert';
import FastGPTLink from '@/components/docs/linkFastGPT';

This guide covers how to set up your development environment to build and test <FastGPTLink campaign="docs_self_host_dev" content="intro_product_link">gptGO</FastGPTLink>.

## Prerequisites

Install and configure these dependencies on your machine to build <FastGPTLink campaign="docs_self_host_dev" content="prerequisites_product_link">gptGO</FastGPTLink>:
...
### 1. Fork the gptGO Repository

Fork the [gptGO repository](https://github.com/zeroven0205-spec/).
...
If you're already running gptGO locally via Docker, stop it first to avoid port conflicts.

Navigate to `FastGPT/deploy/dev` and run `docker compose up -d` to start gptGO's dependencies:
...
1. Make sure your code is forked from the [gptGO](https://github.com/zeroven0205-spec/) repository.
2. Keep commits small and focused — each should address one issue.
3. Submit a PR to gptGO's main branch. The gptGO team and community will review it with you.

If you run into issues like merge conflicts, check GitHub's [pull request tutorial](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests). Once your PR is merged, you'll be listed in the [contributors table](https://github.com/zeroven0205-spec/).
...
gptGO runs a `postinstall` script after `pnpm i` to auto-generate ChakraUI types. If you get a permission error, run `chmod -R +x ./scripts/` first, then `pnpm i`.
...
gptGO uses Next.js page routing. To separate frontend and backend code, directories are split into global, service, and web subdirectories for shared, backend-only, and frontend-only code respectively.

### Monorepo

gptGO uses pnpm workspace for its monorepo structure, with two main parts:

- projects/app - gptGO main project
- packages/ - Submodules
...
gptGO's code modules follow DDD principles, divided into these domains:

- core - Core features (Dataset, workflow, app, conversation)
