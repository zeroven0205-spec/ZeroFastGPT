...
description: gptGO usage notes
---

# Troubleshooting Notes

If you encounter issues while using gptGO, follow the steps below to troubleshoot and resolve them.
...
- **Check version:** View the current running version on the gptGO homepage or in the admin panel.
...
gptGO reads the client IP for IP rate limiting, share-link IP allowlists, chat log IP records, and IP geolocation. If your self-hosted gptGO is behind Nginx, a load balancer, an Ingress controller, or a CDN, make sure clients cannot spoof `X-Forwarded-For` or `X-Real-IP` headers.
...
- **Enable trusted proxy validation in gptGO:** trust forwarded IP headers only when they come from Nginx, the load balancer, or the Ingress controller.
- **Restrict direct access to gptGO:** firewall or security group rules should allow only the reverse proxy to access the gptGO service port.

gptGO environment variable example:
...
`TRUSTED_PROXY_IPS` should contain the previous-hop proxy IP or CIDR that gptGO sees directly, such as the Docker subnet for the Nginx container, the Ingress Controller private address, or the load balancer origin address. Do not use a trust-all CIDR such as `0.0.0.0/0` because it would trust every source, and do not add normal client networks to the trusted list.
...
If a CDN or load balancer is in front of Nginx, configure Nginx to trust only those upstream egress IPs first. Then forward the restored client IP to gptGO:
...
```bash
curl -H 'X-Forwarded-For: 6.6.6.6' -H 'X-Real-IP: 6.6.6.6' https://gptgo.example.com
```

If the configuration is correct, gptGO should still record and validate the real client IP, not the spoofed value `6.6.6.6` from the request.
