...
您需要在计算机上安装和配置以下依赖项才能构建 <FastGPTLink campaign="docs_self_host_dev" content="prerequisites_product_link">gptGO</FastGPTLink>：
...
您需要 Fork [gptGO 存储库](https://github.com/zeroven0205-spec/)。
...
若您本地已经通过 docker 启动了 gptGO，则需要先关闭，否则会有端口冲突。

切换到 `FastGPT/deploy/dev` 目录，执行 `docker compose up -d` 运行 gptGO 的各种依赖。
...
1. 确保你的代码是 Fork [gptGO](https://github.com/zeroven0205-spec/) 仓库
2. 尽可能少量的提交代码，每次提交仅解决一个问题。
3. 向 gptGO 的 main 分支提交一个 PR，提交请求后，gptGO 团队/社区的其他人将与您一起审查它。

如果遇到问题，比如合并冲突或不知道如何打开拉取请求，请查看 GitHub 的[拉取请求教程](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests)，了解如何解决合并冲突和其他问题。一旦您的 PR 被合并，您将自豪地被列为[贡献者表](https://github.com/zeroven0205-spec/)中的一员。
...
gptGO 在 `pnpm i` 后会执行 `postinstall` 脚本，用于自动生成 `ChakraUI` 的 `Type`。如果没有权限，可以先执行 `chmod -R +x ./scripts/`，再执行 `pnpm i`。
...
gptGO 使用了 nextjs 的 page route 作为框架。为了区分好前后端代码，在目录分配上会分成 global, service, web 3 个自目录，分别对应着 `前后端共用`、`后端专用`、`前端专用` 的代码。

### monorepo

gptGO 采用 pnpm workspace 方式构建 monorepo 项目，主要分为两个部分：

- projects/app - gptGO 主项目
- packages/ - 子模块
...
gptGO 在代码模块划分时，按 DDD 的思想进行划分，主要分为以下几个领域：
