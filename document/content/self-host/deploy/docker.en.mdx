...
description: Quickly deploy gptGO using Docker Compose
---
...
- AI Proxy: Required for proxying model requests and managing providers in the Admin interface.
...
Non-interactive mode also requires `FASTGPT_FE_DOMAIN`, the full URL users use to access gptGO, such as `https://fastgpt.example.com`. Versions with Agent/Skill Sandbox also require `FASTGPT_SANDBOX_PROXY_URL`, the Sandbox WebSocket URL, such as `wss://sandbox-proxy.example.com`; version 4.16 additionally requires `FASTGPT_SANDBOX_PREVIEW_PROXY_URL` for the HTTP preview URL. Version 4.14 does not include this service. In interactive mode, the script prompts for the addresses required by each version; 4.15 prompts only for the WebSocket URL.

Access URLs and host ports are configured independently. For direct access, set `FASTGPT_PORT` for the gptGO host port (default `3000`) and `FASTGPT_SANDBOX_PROXY_PORT` for the Sandbox Proxy host port (default `3006`); the container ports remain `3000` and `1006`. With a reverse proxy, public ports in `FASTGPT_FE_DOMAIN`, `FASTGPT_SANDBOX_PROXY_URL`, and the preview URL (for example, `443`) are not treated as host ports, so the script keeps the default Compose mappings.
...
- **Milvus**
  - China mirror (Alibaba Cloud): [docker-compose.milvus.yml](/deploy/docker/main/cn/docker-compose.milvus.yml)
  - Global mirror (dockerhub, ghcr): [docker-compose.milvus.yml](/deploy/docker/main/global/docker-compose.milvus.yml)
- **Zilliz**
...
You must set `FE_DOMAIN` in `fastgpt-app` to the full URL users use to access gptGO, such as `https://fastgpt.example.com`. It must include a scheme, host, and optional port; do not leave it empty or use an internal container address.

When Agent/Skill Sandbox is enabled, also configure:

- `AGENT_SANDBOX_PROXY_URL`: the browser-accessible Sandbox Proxy WebSocket URL using `ws://` or `wss://`, such as `wss://sandbox-proxy.example.com`, which must be forwarded to the Sandbox Proxy host port (default `3006`).
- Version 4.16 additionally requires `AGENT_SANDBOX_PREVIEW_PROXY_URL`: the browser-accessible HTTP(S) URL for sandbox file previews, such as `https://sandbox-proxy.example.com`, forwarded to the same host port.
...
The following services must be reachable. For direct access, open the host ports printed by the script; with a reverse proxy, the proxy owns the public ports:

1. Port 3000 by default (gptGO main service; change the host port with `FASTGPT_PORT`)
2. Port 9000 (S3 service)
3. Port 3006 by default (gptGO Agent Sandbox Proxy service; change the host port with `FASTGPT_SANDBOX_PROXY_PORT`)
4. Port 3003 (gptGO SSE MCP server service; you can leave this port closed if you don't need it.)
...
Access gptGO via the port/domain opened in step 3.
...
- After first login, the system prompts that `Language Model` and `Embedding model` are not configured and automatically redirects to the model configuration page. At least these two model types are required.
- If the redirect doesn't happen, go to `Admin - Model provider` to configure models. [View tutorial](../config/model/intro.en.mdx)
...
Starting from V4.14.0, the gptGO-plugin image only provides the runtime environment without pre-installed system plugins. All gptGO systems must manually install system plugins.

- Install via the plugin marketplace — by default it fetches from the public gptGO Marketplace.
- If your gptGO can't access the marketplace, visit [gptGO Plugin Marketplace](https://github.com/zeroven0205-spec/), download .pkg files, and import them via file upload.
...
### gptGO and gptGO-plugin Version Compatibility

| gptGO-plugin Version | gptGO Main Service   |
| -------------------- | -------------------- |
| 1.x                  | 4.15.x               |
...
Check the `STORAGE_EXTERNAL_ENDPOINT` variable — it must be accessible by both the client and gptGO service.
...
Starting with V4.17.0, **you must deploy or connect to AI Proxy** and set these environment variables in the gptGO container:

- `AIPROXY_API_ENDPOINT`: The AI Proxy service root URL reachable from gptGO, such as `http://aiproxy:3000`. Do not append `/v1`; gptGO adds the path required by each API.
- `AIPROXY_API_TOKEN`: A nonempty AI Proxy administrator token that matches the administrator credentials configured on the AI Proxy server.

Startup validation fails if either setting is missing. `OPENAI_BASE_URL` and `CHAT_API_KEY` do not replace these required settings. AI Proxy is still required when a model uses `requestUrl/requestAuth`. Configure model service endpoints and keys in the Admin provider settings.

To update the AI Proxy connection, edit the gptGO container's environment variables in `docker-compose.yml`. Commercial Admin services that share App/Service configuration also need both variables.
...
PG database not connected or initialization failed — check logs. gptGO initializes tables on each PG connection. Errors will appear in the logs.

1. Check if the database container started normally
2. For non-Docker deployments, manually install the pg vector extension
3. Check gptGO logs for related errors
