---
title: Sealos 部署
description: 使用 Sealos 一键部署 gptGO
---
...
gptGO 使用了 one-api 项目来管理模型池，其可以兼容 OpenAI、Azure、国内主流模型和本地模型等。
...
点击详情后，会跳转到 gptGO 的部署管理页面，点击外网访问地址中的链接，即可打开 gptGO 服务。
...
gptGO 商业版共包含了 2 个应用（gptGO, gptGO-plus）和 2 个数据库，使用多 API Key 时候需要安装 OneAPI（一个应用和一个数据库），总计 3 个应用和 3 个数据库。
...
在 Sealos 中，你可以打开 `应用管理`（App Launchpad）看到部署的 gptGO，可以打开 `数据库`（Database）看到对应的数据库。

在 `应用管理` 中，选中 gptGO，点击变更，可以看到对应的环境变量和配置文件。

<Alert icon="🤖" context="success">
  在 Sealos 上，gptGO 一共运行了 1 个服务和 2
  个数据库，如暂停和删除请注意数据库一同操作。（你可以白天启动，晚上暂停它们，省钱大法）
</Alert>

### 如何更新/升级 gptGO

[升级脚本文档](../upgrading/upgrade-instruction.mdx)先看下文档，看下需要升级哪个版本。注意，不要跨版本升级。
