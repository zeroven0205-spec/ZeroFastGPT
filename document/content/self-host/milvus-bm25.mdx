...
description: '将 Milvus 升级到 2.5.16+ 并启用 gptGO BM25 全文检索'
---

## 背景

gptGO 的全文检索默认走 MongoDB `$text`。当使用 **Milvus 作为向量库**时，全文检索自动切换到 Milvus BM25（`modeldata_v2` 单表，向量 + 全文同表），不再写 MongoDB 全文表。这要求 Milvus 版本 **≥ 2.5.16**；低于 2.5.16 时 gptGO 启动会直接报错退出，不会降级运行。
...
建议在**低写入时段**执行，或先停止 gptGO 应用写入。迁移期间如需保持服务，请勿同时写数据集数据（详见第 5 步"迁移期间新数据"）。
...
启动 gptGO，Milvus 初始化时会调用 `getVersion()` 校验版本；低于 2.5.16、无法获取或无法解析版本会**终止启动**。也可以手动验证：
...
> 删除后 gptGO 重启**不会**重新创建或访问旧表：正常初始化只创建/加载 `modeldata_v2`，`modeldata` 仅由迁移脚本探测/加载。

### 8. 回滚

- 未执行 `removeOld`（旧表仍在）：降级 gptGO 镜像并恢复备份即可，旧全文数据仍在 MongoDB。
