...
description: 'Upgrade Milvus to 2.5.16+ and enable gptGO BM25 full-text search'
---

## Background

gptGO full-text search defaults to MongoDB `$text`. When **Milvus is used as the vector store**, full-text search automatically switches to Milvus BM25 (single `modeldata_v2` table, vector + full-text in one collection) and stops writing the MongoDB full-text table. This requires Milvus **≥ 2.5.16**; below 2.5.16 gptGO fails to start with an explicit error — it never silently downgrades.
...
Run during a **low-write window**, or stop gptGO app writes first. If you must keep serving during migration, avoid writing dataset data concurrently (see step 5 "New data during migration").
...
Start gptGO — Milvus initialization calls `getVersion()` to gate the version; below 2.5.16, or when the version cannot be fetched/parsed, **startup terminates**. You can also verify manually:
...
> After deletion, gptGO restart does **not** recreate or access the old table: normal init only creates/loads `modeldata_v2`; `modeldata` is detected/loaded only by the migration script.

### 8. Rollback

- If `removeOld` was **not** run (old table intact): downgrade the gptGO image and restore backups; the legacy full-text data is still in MongoDB.
...
- **Migration reports that the old `modeldata` collection is missing or empty**: stop the migration and check whether gptGO is connected to the correct Milvus instance and whether the data volume is mounted correctly. Restore from backup if the data is lost.
