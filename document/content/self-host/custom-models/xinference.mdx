...
- [Transformers](https://huggingface.co/docs/transformers/index)：通过集成 Huggingface 的 Transformers 库作为后端，Xinference 可以最快地集成当今自然语言处理（NLP）领域的最前沿模型（自然也包括 LLM）。
...
PyPi 在安装 Transformers 和 vLLM 时会自动安装 PyTorch，但自动安装的 CUDA 版本可能与你的环境不匹配，此时你可以根据 PyTorch 官网中的[安装指南](https://pytorch.org/get-started/locally/)来手动安装。
...
Xinference 默认会在本地启动服务，端口默认为 9997。因为这里配置了 -H 0.0.0.0 参数，非本地客户端也可以通过机器的 IP 地址来访问 Xinference 服务。
...
![](../../../public/imgs/xinference-launch-model.png)
...
![](../../../public/imgs/one-api-add-xinference-models.jpg)
...
修改 gptGO 的 `config.json` 配置文件的 llmModels 部分加入 qwen-chat 模型：
...
然后重启 gptGO 就可以在应用配置中选择 Qwen 模型进行对话：

##

- 参考：[gptGO + Xinference：一站式本地 LLM 私有化部署和应用开发](https://github.com/zeroven0205-spec/)
