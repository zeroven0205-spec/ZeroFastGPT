...
在 `gptGO v4.9.0` 版本中，社区版用户可以在 `config.json` 文件中添加 `systemEnv.customPdfParse` 配置，来使用 Marker 解析 PDF 文件。商业版用户直接在 Admin 后台根据表单指引填写即可。需重新拉取 Marker 镜像，接口格式已变动。

## 使用教程

### 1. 安装 Marker

参考文档 [Marker 安装教程](https://github.com/zeroven0205-spec/)，安装 Marker 模型。封装的 API 已经适配了 gptGO 自定义解析服务。
...
gptGO V4.9.0 版本之前，可以用以下方式，试用 Marker 解析服务。
...
- CUSTOM_READ_FILE_URL - 自定义解析服务的地址, host 改成解析服务的访问地址，path 不能变动。
