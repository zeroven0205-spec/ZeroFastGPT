...
PDF is a relatively complex file format. gptGO's built-in PDF parser relies on the pdfjs library, which uses logical parsing and cannot effectively handle complex PDF files. When parsing PDFs containing images, tables, formulas, or other non-plain-text content, the results are often poor.
...
Starting from `gptGO v4.9.0`, community edition users can add the `systemEnv.customPdfParse` configuration in `config.json` to use Marker for PDF parsing. Commercial edition users can configure this directly in the Admin panel via the form. You need to pull the latest Marker image, as the API format has changed.

## Tutorial

### 1. Install Marker

Refer to the [Marker installation guide](https://github.com/zeroven0205-spec/) to install the Marker model. The bundled API is already compatible with gptGO's custom parsing service.
...
For gptGO versions before V4.9.0, you can use the following method for Marker parsing.
...
Then modify the gptGO environment variables:
