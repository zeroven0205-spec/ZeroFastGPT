...
description: gptGO model configuration guide
---
...
gptGO uses the `AI Proxy` service to connect to different model providers. AI Proxy also provides load balancing, model logging, and analytics dashboards to help you monitor model usage.
...
You can configure models from `Admin - Model provider` in gptGO.
...
4. Model Mapping: Maps the model name in gptGO requests to the actual model name at the provider. For example:
...
In gptGO, the model is `gpt-4o-test`, and requests to AI Proxy also use `gpt-4o-test`. When AI Proxy forwards the request upstream, the actual `model` value becomes `gpt-4o`.
...
gptGO provides simple tests for each model type on the UI to verify that models are working correctly. Each test sends an actual request using a template.
...
```json
{
  ...
  "metadata": {
    ...
    "provider": "OpenAI", // Model provider, used for categorization. Built-in providers: https://github.com/zeroven0205-spec/ You can submit a PR for new providers, or use "Other"
    ...
    "charsPointsPrice": 0, // Points per 1k tokens (commercial edition)
...
```json
{
  ...
  "metadata": {
    ...
    "charsPointsPrice": 0, // Points per 1k tokens
