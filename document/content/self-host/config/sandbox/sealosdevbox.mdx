...
description: gptGO 使用 Sealos Devbox 沙盒
---
...
1. 已部署 gptGO 商业版，并确认团队拥有 Agent Sandbox 使用权限。
...
对于 root 权限的 Agent 沙箱，gptGO 会根据发行版生成并覆盖标准的 Ubuntu 或 Debian `.sources` 文件，并在覆盖已有文件前创建 `.copy` 备份。删除该环境变量后，有 `.copy` 备份的文件会恢复，没有备份的文件会跳过。初始化阶段不会执行 `apt-get update`。arm64 运行镜像应配置提供对应架构软件包的镜像地址。
...
`sealosdevbox` provider 启用后必须配置 `AGENT_SANDBOX_SEALOS_IMAGE`。请使用客服提供或与当前 gptGO 版本匹配的 Agent Sandbox 运行态镜像。

### 浏览器 WebSocket 连接失败

检查代理服务是否能被浏览器访问，并确认反向代理已支持 WebSocket Upgrade。如果 gptGO 通过 HTTPS 访问，`AGENT_SANDBOX_PROXY_URL` 也应使用 `wss://`，避免浏览器拦截混合内容。

### proxy 校验失败或返回 401

确认 gptGO 主服务和 `fastgpt-agent-sandbox-proxy` 中的 `AGENT_SANDBOX_PROXY_SECRET` 完全一致，并且长度不少于 32 位。
