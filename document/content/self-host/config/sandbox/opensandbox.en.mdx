...
description: Configure OpenSandbox and Agent Sandbox Proxy for gptGO
---
...
OpenSandbox is designed for self-hosted Agent and Skill sandbox runtimes. gptGO creates sandboxes through OpenSandbox Server, while Agent Sandbox Proxy provides browser access to files, terminals, and previews.
...
| Setting                       | Description                                                                                                                  |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `x-volume-manager-auth-token` | Volume Manager token. It must match `AGENT_SANDBOX_OPENSANDBOX_VOLUME_MANAGER_TOKEN` in gptGO.                               |
| `[server].api_key`            | OpenSandbox Server API key. It must match `AGENT_SANDBOX_OPENSANDBOX_API_KEY` in gptGO.                                      |
...
| `AGENT_SANDBOX_PROXY_SECRET`       | None                                     | Secret shared with the gptGO main service. Must be at least 32 characters.                                                      |
| `FASTGPT_APP_URL`                  | `http://fastgpt-app:3000`                | Internal gptGO URL used by the proxy.                                                                                           |
| `FASTGPT_APP_REQUEST_TIMEOUT_SECS` | `10`                                     | Timeout for proxy requests to gptGO, in seconds. Increase for slow cold starts.                                                 |
...
<Alert icon="⚠️" context="warning">
  Host the preview proxy on an origin separate from the gptGO application, using a different scheme,
  host, or port. Sandbox HTML may contain user-generated scripts. If previews share the application
  origin, those scripts may be able to access application credentials or APIs.
</Alert>
...
gptGO reads `/etc/os-release` and generates or overwrites `/etc/apt/sources.list.d/ubuntu.sources` based on the Ubuntu version and codename. The file includes the main repository, updates, backports, and security. Before overwriting an existing sources file, gptGO creates a `.copy` backup. When the variable is removed, files with a backup are restored and files without a backup are left unchanged. Initialization does not run `apt-get update`.
...
Check that the proxy is reachable from the browser and that your reverse proxy supports WebSocket Upgrade. If gptGO uses HTTPS, `AGENT_SANDBOX_PROXY_URL` should use `wss://`.

### Proxy validation fails or returns 401

Make sure `AGENT_SANDBOX_PROXY_SECRET` is identical in gptGO and Agent Sandbox Proxy and contains at least 32 characters.
