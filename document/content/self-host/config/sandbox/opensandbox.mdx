...
description: gptGO OpenSandbox 与 Agent Sandbox Proxy 配置
---
...
OpenSandbox 适合需要自托管 Agent/Skill 沙盒运行环境的场景。gptGO 通过 OpenSandbox Server 创建沙盒，并通过 Agent Sandbox Proxy 为浏览器提供文件、终端和预览访问能力。
...
| 配置                          | 说明                                                                                                             |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `x-volume-manager-auth-token` | Volume Manager 的认证 Token，需要与 gptGO 的 `AGENT_SANDBOX_OPENSANDBOX_VOLUME_MANAGER_TOKEN` 一致。             |
| `[server].api_key`            | OpenSandbox Server API Key，需要与 gptGO 的 `AGENT_SANDBOX_OPENSANDBOX_API_KEY` 一致。                           |
...
| `AGENT_SANDBOX_PROXY_SECRET`       | 无                                       | 与 gptGO 主服务共用的密钥，至少 32 位。                                                                 |
| `FASTGPT_APP_URL`                  | `http://fastgpt-app:3000`                | Proxy 回源 gptGO 主服务的内网地址。                                                                     |
...
<Alert icon="⚠️" context="warning">
  预览代理应部署在与 gptGO 主站不同的 origin（协议、域名或端口至少一项不同）。Sandbox 中的 HTML
...
配置后，gptGO 会读取沙盒的 `/etc/os-release`，根据 Ubuntu 版本和 codename 生成并覆盖 `/etc/apt/sources.list.d/ubuntu.sources`，写入主仓库、updates、backports 和 security。修改配置前，已有 sources 文件会备份为 `.copy`；删除环境变量后，有备份的文件会恢复，没有备份的文件保持不变。初始化阶段不会执行 `apt-get update`。
...
检查 Proxy 是否可从浏览器访问，并确认反向代理支持 WebSocket Upgrade。如果 gptGO 使用 HTTPS，`AGENT_SANDBOX_PROXY_URL` 也应使用 `wss://`。

### Proxy 校验失败或返回 401

确认 gptGO 主服务和 Agent Sandbox Proxy 中的 `AGENT_SANDBOX_PROXY_SECRET` 完全一致，且长度不少于 32 位。
