...
| `PRO_TOKEN`           | Empty              | Token for gptGO app server calls to pro/admin internal APIs. It must match the pro/admin configuration and is required when App configures `PRO_URL`. |
...
Starting with V4.17.0, AI Proxy is required. App and Admin services that share App/Service configuration must set `AIPROXY_API_ENDPOINT` and a nonempty `AIPROXY_API_TOKEN`. Startup validation fails if either is missing. Legacy OpenAI environment variables and per-model custom request URLs do not replace these settings.

| Variable                 | Default                           | Description                                                                                               |
| ------------------------ | --------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `PLUGIN_BASE_URL`        | `http://localhost:3004`           | gptGO Plugin service URL. Deployment templates usually set this to the internal Plugin service URL.       |
...
| `AIPROXY_API_ENDPOINT`   | None, **required**                | AI Proxy service root URL, such as `http://aiproxy:3000`. Do not append `/v1`.                            |
| `AIPROXY_API_TOKEN`      | None, **required**                | A nonempty AI Proxy administrator token matching the server configuration.                                |
| `CRM_API_URL`            | Empty                             | Lead attribution CRM API base URL (including `/api/v1`). Empty disables identity reporting.               |
| `CRM_API_KEY`            | Empty                             | CRM admin API key used to bind a gptGO user to `visitor_id` after registration or login.                  |
| `MARKETPLACE_URL`        | `https://marketplace.example.com` | Plugin marketplace API URL.                                                                               |
...
| `AGENT_SANDBOX_OPENSANDBOX_VOLUME_NAME_PREFIX`   | `fastgpt-session`        | Prefix used by the gptGO app when generating persistent OpenSandbox volume `claimName` values. When upgrading, reuse the previous `VM_VOLUME_NAME_PREFIX` value.                                                                                                                                                                                              |
...
| `AGENT_SANDBOX_PREVIEW_PROXY_URL`                | Empty                    | Browser-accessible HTTP(S) URL for Sandbox file previews. You must add it to both `fastgpt-app` and `fastgpt-pro` when Agent Sandbox is enabled. Use an origin separate from the gptGO app.                                                                                                                                                                   |
| `AGENT_SANDBOX_FREE_TIP`                         | `false`                  | Whether the frontend shows the Agent Sandbox free-use hint.                                                                                                                                                                                                                                                                                                   |
| `AGENT_SANDBOX_STORAGE_SIZE_GI`                  | `1`                      | Agent Sandbox storage size in Gi. gptGO derives the archive, Skill, and single-file limits from this value.                                                                                                                                                                                                                                                   |
...
| `STORAGE_DOWNLOAD_URL_MODE`             | `short-proxy`           | Download mode: `short-proxy` or `short-redirect`. External URLs are always gptGO short links.         |
...
| Variable                  | Default                     | Description                                                                                                                                                                 |
| ------------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FE_DOMAIN`               | Required                    | The origin clients use to access gptGO, including the scheme, host, and optional port. It completes file and image URLs. Local development can use `http://localhost:3000`. |
| `FILE_DOMAIN`             | Empty                       | File access domain. It usually points to gptGO, but a separate domain can isolate file risk.                                                                                |
| `FILE_URL_EXPIRED_DAYS`   | `90`                        | Lifetime in days of file access short links persisted in chat records. Fractions are allowed.                                                                               |
...
| `DEFAULT_TEAM_BASIC_PERMISSIONS_ENABLED` | `false`     | Whether newly created teams grant their default all-members group permission to create Apps, Skills, and Datasets. Does not change existing teams. Configure consistently in gptGO and Pro Admin. |
...
| `SYSTEM_MIGRATION_BATCH_SIZE`            | `100`       | Number of records processed per system migration batch. Valid range: `50`–`1000`.                                                                                                                 |
...
The open-source edition supports custom PDF parsing services, SoMark, TextIn, and Doc2x. Configure only one service. If you configure more than one, gptGO uses this priority order: custom PDF parsing service, SoMark, TextIn, then Doc2x.
...
`CUSTOM_PDF_PARSE_KEY` is optional. When set, gptGO sends it to the parsing service as `Authorization: Bearer <CUSTOM_PDF_PARSE_KEY>`. The service must accept a `multipart/form-data` POST request with a `file` field and return JSON in this format:
...
Restart gptGO after changing the environment variables. Then enable **Enhanced PDF Parsing** when importing files into a Dataset or configuring App file uploads. PDFs use the configured enhanced parsing service only when this option is enabled; otherwise, gptGO uses its built-in parser.
...
| Variable                              | Default            | Description                                                                                                            |
| ------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `PRO_TOKEN`                           | None, **required** | Service-to-service token for gptGO app calls to pro/admin internal APIs. Must be at least 32 characters and match App. |
...
| `SANDBOX_POOL_SIZE`               | `5`                                                      | Number of pre-warmed JS/Python workers, from `1` to `100`.                                                                                        |
...
These variables are loaded and validated by `projects/volume-manager/src/env.ts`. The `AGENT_SANDBOX_OPENSANDBOX_VOLUME_MANAGER_TOKEN` used by gptGO for persistent OpenSandbox volumes must match `VM_AUTH_TOKEN`.
