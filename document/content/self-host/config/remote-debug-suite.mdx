...
description: gptGO 接入系统插件的远程调试功能套件
---
...
系统插件的远程调试功能套件用于把开发者本地运行的 gptGO 系统插件临时接入 gptGO 测试环境。它适合系统插件开发、联调和验收，不适合作为生产插件运行时。

<Alert icon="🤖" context="warning">

系统插件的远程调试功能套件仅商业版支持。

优先推荐在 gptGO 云服务版本中使用远程调试能力。自部署需要额外维护 Plugin Server、Connection Gateway、Redis、反向代理、TLS 和密钥轮换，运维成本更高。

</Alert>

默认的 Docker Compose 部署脚本只包含 gptGO 主服务和常规 `fastgpt-plugin` 运行环境，不包含 Connection Gateway 的公网 WebSocket 接入配置。自部署环境需要按本文额外部署系统插件的远程调试功能套件。
...
| 组件                 | 作用                                                            |
| -------------------- | --------------------------------------------------------------- |
| gptGO 主服务         | 提供开启、刷新、关闭调试通道的页面和 API。                      |
...
1. gptGO 主服务已能正常访问 `fastgpt-plugin`，并且两侧的 `PLUGIN_TOKEN` / `AUTH_TOKEN` 一致。
2. `fastgpt-plugin` 版本需要包含远程调试能力；建议与当前 gptGO 版本要求的 plugin 版本保持一致。
...
`NEXT_PUBLIC_BASE_URL` 会影响调试连接链接的生成。公网用户访问 gptGO 时，应配置为浏览器可访问的 gptGO 地址。
...
1. 在 gptGO 插件调试入口开启调试通道，复制页面返回的连接链接。
2. 在本地插件目录运行：
...
连接成功后，本地 CLI 会通过 Gateway 上报插件 metadata。gptGO 工具列表中会出现当前调试 source 下的本地插件。调试 source 的格式为：
...
2. 在 gptGO 页面开启调试通道，确认状态从 `enabled` 变为 `connected`。
3. 运行本地 `fastgpt-plugin dev`，确认 CLI 显示 WebSocket 已连接。
4. 在 gptGO 中选择调试 source 下的工具并触发一次调用，确认结果由本地插件返回。
...
### CLI 已连接，但 gptGO 显示 disconnected
