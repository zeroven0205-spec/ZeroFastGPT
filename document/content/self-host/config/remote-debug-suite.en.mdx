...
description: Configure the system plugin remote debugging suite for self-hosted gptGO deployments
---
...
The system plugin remote debugging suite temporarily connects gptGO system plugins running on a developer's local machine to a gptGO test environment. It is intended for system plugin development, integration testing, and acceptance checks, not as a production plugin runtime.

<Alert icon="🤖" context="warning">

The system plugin remote debugging suite is available only in the commercial edition.

We recommend using remote debugging in the gptGO Cloud version first. Self-hosted deployments require you to operate Plugin Server, Connection Gateway, Redis, reverse proxy, TLS, and secret rotation yourself.

</Alert>

The default Docker Compose deployment only includes the gptGO main service and the regular `fastgpt-plugin` runtime. It does not include the public WebSocket setup required by Connection Gateway. For self-hosted deployments, deploy the system plugin remote debugging suite separately.
...
| Component            | Purpose                                                                                 |
| -------------------- | --------------------------------------------------------------------------------------- |
| gptGO main service   | Provides the UI and APIs for enabling, refreshing, and revoking a debug channel.        |
...
1. The gptGO main service can access `fastgpt-plugin`, and `PLUGIN_TOKEN` / `AUTH_TOKEN` are the same on both sides.
2. Your `fastgpt-plugin` version includes remote debugging. Use the plugin version required by your current gptGO release.
...
## Configure gptGO Main Service

The gptGO main service keeps using the regular plugin configuration:

```dotenv
PLUGIN_BASE_URL=http://fastgpt-plugin:3000
PLUGIN_TOKEN=replace-with-the-same-value-as-plugin-auth-token
NEXT_PUBLIC_BASE_URL=https://gptgo.example.com
```

`NEXT_PUBLIC_BASE_URL` affects the generated debug connection link. For public access, set it to the gptGO URL reachable by the browser.
...
1. Enable the debug channel from the gptGO plugin debug entry and copy the generated connection link.
...
After the connection succeeds, the local CLI reports plugin metadata through Gateway. The local plugins appear in gptGO under the current debug source. The debug source format is:
...
2. Enable the debug channel in gptGO and confirm the status changes from `enabled` to `connected`.
3. Run `fastgpt-plugin dev` locally and confirm the CLI reports an active WebSocket connection.
4. Select a tool under the debug source in gptGO and invoke it once. The result should come from the local plugin.
...
### The CLI is connected, but gptGO shows disconnected
...
### Tool call times out after connection
