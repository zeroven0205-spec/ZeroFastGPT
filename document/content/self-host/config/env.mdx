...
本文说明 gptGO 自部署时常用服务的环境变量。`projects/app` 与 `pro/admin` 会大量复用 `packages/service/env.ts` 中的服务端配置，因此数据库、密钥、对象存储、向量库等变量合并说明；只有 `projects/app` 或 `pro/admin` 自己读取的变量单独列出。
...
| `PRO_TOKEN`           | 空                 | gptGO app 服务端调用 pro/admin 内部接口的凭证，需与 pro/admin 配置一致；App 配置 `PRO_URL` 时必填。 |
| `PRO_URL`             | 空                 | 商业版服务地址，配置后 App 可调用 Pro API，也会作为文件 URL 安全校验允许域名。                      |

### 服务地址与集成

从 V4.17.0 起，AI Proxy 为必需依赖。App 和复用 App/Service 配置的 Admin 服务都必须设置 `AIPROXY_API_ENDPOINT` 和非空的 `AIPROXY_API_TOKEN`；缺少任一项会导致启动校验失败。旧 OpenAI 环境变量或单模型自定义请求地址不能替代这两个配置。

| 变量                     | 默认值                            | 说明                                                                          |
| ------------------------ | --------------------------------- | ----------------------------------------------------------------------------- |
| `PLUGIN_BASE_URL`        | `http://localhost:3004`           | gptGO Plugin 服务地址；部署模板通常会配置为内部 Plugin 服务地址。             |
...
| `AIPROXY_API_ENDPOINT`   | 无，**必填**                      | AI Proxy 服务根地址，例如 `http://aiproxy:3000`，不要附加 `/v1`。             |
| `AIPROXY_API_TOKEN`      | 无，**必填**                      | AI Proxy 管理员 Token，必须与服务端配置一致且不能为空。                       |
| `CRM_API_URL`            | 空                                | 官网访客归因 CRM 的 API 基础地址（包含 `/api/v1`）；为空时不进行身份上报。    |
| `CRM_API_KEY`            | 空                                | CRM 管理 API Key，用于注册或登录成功后按 `visitor_id` 绑定 gptGO 用户。       |
| `MARKETPLACE_URL`        | `https://marketplace.example.com` | 插件市场接口地址。                                                            |
...
| `AGENT_SANDBOX_OPENSANDBOX_VOLUME_NAME_PREFIX`   | `fastgpt-session`        | gptGO app 生成 OpenSandbox 持久卷 `claimName` 时使用的前缀；升级时需沿用旧 `VM_VOLUME_NAME_PREFIX` 的值。                                                                                                                                                          |
...
| `AGENT_SANDBOX_PREVIEW_PROXY_URL`                | 空                       | 浏览器访问 Sandbox 文件预览的 HTTP(S) 地址；`fastgpt-app` 和 `fastgpt-pro` 启用 Agent Sandbox 时都必须增加。建议使用与 gptGO 主站不同的 origin。                                                                                                                   |
| `AGENT_SANDBOX_FREE_TIP`                         | `false`                  | 前端是否展示 Agent Sandbox 免费提示。                                                                                                                                                                                                                              |
| `AGENT_SANDBOX_STORAGE_SIZE_GI`                  | `1`                      | Agent Sandbox 存储容量，单位 Gi；gptGO 根据该值计算归档、Skill 和单文件限制。                                                                                                                                                                                      |
...
| `STORAGE_DOWNLOAD_URL_MODE`             | `short-proxy`           | 下载模式，可选 `short-proxy` 或 `short-redirect`；对外始终返回 gptGO 短链。                    |
...
| 变量                      | 默认值                      | 说明                                                                                                                                |
| ------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `FE_DOMAIN`               | 必填                        | 客户端访问 gptGO 时使用的地址（由协议、主机和可选端口组成），用于补全文件、图片等资源路径；本地开发可使用 `http://localhost:3000`。 |
| `FILE_DOMAIN`             | 空                          | 文件访问域名，通常也指向 gptGO 服务；可独立域名隔离文件风险。                                                                       |
| `FILE_URL_EXPIRED_DAYS`   | `90`                        | 聊天记录中持久化文件访问短链的有效期，单位天，支持小数。                                                                            |
...
| `DEFAULT_TEAM_BASIC_PERMISSIONS_ENABLED` | `false`     | 新建团队时是否为默认全员组授予应用、Skill 和知识库创建权限，不影响已有团队。gptGO 与 Pro Admin 服务应保持配置一致。 |
| `SKIP_FILE_TYPE_CHECK`                   | `false`     | 是否跳过上传文件类型检查。                                                                                          |
| `WECHAT_CHANNEL_CONCURRENCY`             | `1000`      | 微信渠道 poll worker 并发数，最小 `10`。                                                                            |
| `SYSTEM_MIGRATION_BATCH_SIZE`            | `100`       | 系统迁移任务每批处理的记录数，范围 `50`～`1000`。                                                                   |
...
`CUSTOM_PDF_PARSE_KEY` 可选。配置后，gptGO 会通过 `Authorization: Bearer <CUSTOM_PDF_PARSE_KEY>` 请求解析服务。解析服务需接收包含 `file` 字段的 `multipart/form-data` POST 请求，并返回以下 JSON：
...
修改环境变量后需要重启 gptGO。然后在知识库导入文件或应用文件上传配置中勾选“PDF 增强解析”，上传的 PDF 才会使用已配置的增强解析服务；未勾选时仍使用 gptGO 内置解析器。
...
| 变量                                  | 默认值       | 说明                                                                             |
| ------------------------------------- | ------------ | -------------------------------------------------------------------------------- |
| `PRO_TOKEN`                           | 无，**必填** | gptGO app 服务端调用 pro/admin 内部接口的服务间凭证，至少 32 位，需与 App 一致。 |
...
| `SANDBOX_POOL_SIZE`               | `5`                                                     | JS/Python 预热 worker 数量，范围 `1` 到 `100`。                                               |
...
这些变量由 `projects/volume-manager/src/env.ts` 加载和校验。gptGO 侧 OpenSandbox 持久化 Volume 使用的 `AGENT_SANDBOX_OPENSANDBOX_VOLUME_MANAGER_TOKEN` 需要与这里的 `VM_AUTH_TOKEN` 保持一致。
