---
title: 'V4.16.0'
description: 'gptGO V4.16.0 更新说明'
...
访问地址以 `http://` 或 `https://` 开头。单端口部署时，它可以与 `AGENT_SANDBOX_PROXY_URL` 指向同一域名和端口，但协议分别使用 HTTP(S) 和 WebSocket(S)。强烈建议配置的域名与 gptGO 主站使用不同的 origin。同源部署会让这些脚本进入主站的同源安全边界，可能访问主站凭证或接口。系统当前不会强制检查 origin 是否隔离。

可以通过访问: `https://{{host}}/health` 来确认是否可访问。

### 2. 更新 gptGO 环境变量(启用沙盒的需更新)
...
> gptGO、`fastgpt-agent-sandbox-proxy` 和 `fastgpt-agent-sandbox` 的预览协议已同步变更。启用 Agent Sandbox 时必须使用本版本配套镜像，不支持新旧版本混合部署。

### 3. 镜像更新

- 更新 gptGO-app(gptGO 主服务) 镜像 tag: v4.16.0
- 更新 gptGO-pro(gptGO 商业版) 镜像 tag: v4.16.0
- 更新 gptGO-plugin 镜像 tag: v1.1.0
...
- **执行正式迁移前（`dryRun: true`）**：重点关注 `normalization.pendingCount`。该字段表示字段归一化和旧 Skill Debug Chat 清理后仍待处理的数据总数，必须为 `0` 才能开始正式迁移。如果大于 `0`，再通过 `normalization.sandboxPendingCount`、`normalization.legacyDebugChatCleanup.pendingChatCount` 和 `normalization.legacyDebugChatCleanup.list` 定位待处理数据；此时 `normalizationBlocked` 应为 `true`，不会进入 Workspace 归档和后续迁移。dry-run 只做前置清洗统计，返回的 `failedCount`、`failures` 和 `skipped` 不用于判断此前或其他请求中的正式迁移状态。
- **执行升级后（`dryRun: false`）**：先确认前置清洗成功，即 `normalization.pendingCount` 为 `0` 且 `normalizationBlocked` 为 `false`；再确认正式迁移成功，即 `failedCount` 为 `0`。同时满足这三个条件时，表示所有未跳过的 Sandbox 已迁移完成，这是迁移成功的明确判定条件。
- **未完成和失败字段**：`normalizationBlocked: true` 或 `normalization.pendingCount > 0` 表示前置清洗未完成，未进入后续迁移；这表示迁移被阻断，不等同于正式迁移失败。正式迁移失败时，`failedCount > 0`，具体记录查看 `failures`；如果失败发生在前置清洗阶段，也可查看 `normalization.failures`。
- **跳过字段**：仅在使用 `skipError: true` 且源 App 或 Skill 已缺失或软删除时，相关记录才会进入 `skipped`，数量由 `skippedCount` 表示。这些记录会保留且不会迁移，因此 `skippedCount > 0` 不属于迁移失败，但也不表示所有 Legacy 记录都已完成迁移；如需确认没有任何记录被跳过，应额外确认 `skippedCount` 为 `0`。

如果接口返回超时错误，不代表迁移一定失败。正式迁移使用全局任务锁并支持安全重试；请等待原请求对应的任务结束后，再重复执行 `dryRun: false` 获取最新结果。若重试时提示任务锁仍被占用，说明上一轮迁移仍在执行，请稍后再试。已完成的归档和迁移不会重复执行。`dryRun: true` 只能用于检查前置清洗状态，不能用于判断上一轮正式迁移的 `failedCount`、`failures` 或 `skipped`。
