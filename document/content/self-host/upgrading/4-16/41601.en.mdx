---
title: 'V4.16.0-beta1'
description: 'gptGO V4.16.0-beta1 release notes'
...
The access URL must start with `http://` or `https://`. With a single-port deployment, it can use the same host and port as `AGENT_SANDBOX_PROXY_URL`, while the protocols remain HTTP(S) and WebSocket(S), respectively. We strongly recommend using a different origin from the main gptGO site. A same-origin deployment places user-generated scripts inside the main site's security boundary, where they may access site credentials or APIs. gptGO does not currently enforce origin isolation.

Visit `https://{{host}}/health` to verify that the service is accessible.

### 2. Update the gptGO-app environment variables (required when Sandbox is enabled)
...
> The preview protocols have changed for gptGO, `fastgpt-agent-sandbox-proxy`, and `fastgpt-agent-sandbox`. When Agent Sandbox is enabled, use the images released with this version. Mixing old and new versions is not supported.

### 3. Update images

- Update the fastgpt-app (gptGO main service) image tag to `v4.16.0-beta1`
- Update the fastgpt-pro (gptGO commercial edition) image tag to `v4.16.0-beta1`
...
### 6. Other changes (optional)

1. `PASSWORD_LOGIN_LOCK_SECONDS` is deprecated. Use `PASSWORD_LOGIN_MINUTE_LIMIT_COUNT` to control the maximum number of password login attempts allowed per account per minute.
