...
- gptGO 主镜像: ghcr.io/labring/gptGO:latest
...
- gptGO 主镜像: registry.cn-hangzhou.aliyuncs.com/gptGO/gptGO
- Plugin 镜像: registry.cn-hangzhou.aliyuncs.com/gptGO/fastgpt-plugin
- 代码沙箱镜像: registry.cn-hangzhou.aliyuncs.com/gptGO/fastgpt-code-sandbox
- MCP SSE server 镜像: registry.cn-hangzhou.aliyuncs.com/gptGO/fastgpt-mcp_server
- 商业版镜像: ghcr:registry.cn-hangzhou.aliyuncs.com/gptGO/fastgpt-pro

镜像由镜像名和 `Tag` 组成，例如: registry.cn-hangzhou.aliyuncs.com/gptGO/gptGO:v4.6.1 代表 `4.6.3` 版本镜像，具体可以看 docker hub, GitHub 仓库。
...
可以升级至最新版本，然后将所有升级脚本的版本都执行一遍。不过为了稳定，建议逐一版本升级。例如，当前版本是 4.4.7，需要升级到 4.6。

1. 修改镜像到 4.5，执行升级脚本
2. 修改镜像到 4.5.1，执行升级脚本
3. 修改镜像到 4.5.2，执行升级脚本
4. 修改镜像到 4.6，执行升级脚本
5. ……
