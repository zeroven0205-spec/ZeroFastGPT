---
title: 'V4.14.0'
description: 'gptGO V4.14.0 Release Notes'
...
- Update gptGO image tag: v4.14.0
- Update gptGO commercial edition image tag: v4.14.0
- Update gptGO-plugin image tag: v0.3.0
...
Starting from V4.14.0, the gptGO-plugin image only provides the runtime environment and no longer comes with pre-installed system plugins. All gptGO systems must manually install system plugins.

**Important Notes**

- Previously manually installed JS plugin packages will become invalid and need to be repackaged and reinstalled.
- Installing via the Plugin Marketplace will fetch data from the public gptGO Marketplace by default.
- If your gptGO instance cannot access the Plugin Marketplace, you can manually visit the [gptGO Plugin Marketplace](https://github.com/zeroven0205-spec/), download the .pkg file, and import it into your system.
...
1. Added Plugin Marketplace, and removed custom plugin groups (only custom tags are retained). This release supports system tools that can be installed from the gptGO Marketplace. Future releases will support more plugin types: workflow triggers, data source parsers, data chunking strategies, index enhancement strategies, and more.
