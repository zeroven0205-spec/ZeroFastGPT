---
title: 'V4.14.9'
description: 'gptGO V4.14.9 Release Notes'
...
1. Rename the following gptGO environment variables — `SANDBOX_URL` and `SANDBOX_TOKEN` are now `CODE_SANDBOX_URL` and `CODE_SANDBOX_TOKEN`:
...
2. Internal-network security checks are now disabled by default. To re-enable them, set the environment variable `CHECK_INTERNAL_IP=true` (applies to gptGO, fastgpt-pro, and fastgpt-sandbox).

### 2. Update Images

- gptGO image tag: v4.14.9.1
- gptGO Commercial Edition image tag: v4.14.9.1
- gptGO-plugin image tag: v0.5.5
...
5. Updating a single Dataset entry now also refreshes the collection's update timestamp.
6. Form file inputs now support opening files for preview.

## Improvements

1. API-based Dataset sync now has additional fallback methods for retrieving file names.
...
6. System toolkits no longer display a version number (since they have no selectable versions).
...
2. Updating and saving an MCP toolkit could prevent it from being called correctly (due to an incorrect toolId lookup).
3. The search box was missing from the API Dataset file list.
