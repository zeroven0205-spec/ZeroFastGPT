---
title: 'V4.14.11'
description: 'gptGO V4.14.11 Release Notes'
...
- Update gptGO-app (gptGO main service) image tag to: v4.14.11
- Update gptGO-pro (commercial edition) image tag to: v4.14.11
- Update code-sandbox image tag to: v4.14.11
- Update gptGO-plugin image tag to: v0.6.0
...
4. Unified S3 file uploads, with support for proxying S3 uploads and access through gptGO to reduce pre-signed URL configuration issues.
...
2. Refactored model provider management code.
3. Added a default VLM model to the Dataset creation API.
...
3. Fixed a billing error in the API for pushing data to the Dataset.
4. Fixed garbled Chinese characters when uploading Markdown documents to the Dataset, caused by the leading English content being misdetected as `ascii`.
