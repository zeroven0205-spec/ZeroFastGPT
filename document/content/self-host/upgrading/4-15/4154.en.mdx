---
title: 'V4.15.4'
description: 'gptGO V4.15.4 release notes'
...
gptGO services now validate `FE_DOMAIN` at startup. Set it to the origin clients use to access
gptGO, including the scheme, host, and optional port. Use the public client-facing origin in
production; local development can use `http://localhost:3000`.

```bash
FE_DOMAIN=https://gptgo.example.com
```
...
gptGO now performs safe index synchronization automatically at startup:

- Creates indexes that are missing from the current gptGO schemas.
- Removes only built-in historical gptGO indexes explicitly marked as deprecated by the corresponding schema and whose name, key, and relevant options match exactly.
- Preserves custom indexes and any other indexes that are not explicitly declared as deprecated.

This process does not call Mongoose's full `syncIndexes()` operation, so indexes are never removed simply because they are absent from a gptGO schema.

> **Default and deletion boundary: `MONGO_DEPRECATE_INDEX` defaults to `true`. It removes only built-in indexes that a gptGO schema explicitly marks as deprecated and whose definitions match exactly; customer-created indexes are not removed. Give every custom index an explicit name instead of relying on MongoDB's key-derived default name to prevent collisions with built-in gptGO index names.**
...
- Update the `fastgpt-app` (gptGO core service) image tag to `v4.15.4`.
- Update the `fastgpt-pro` (gptGO commercial edition) image tag to `v4.15.4`.
