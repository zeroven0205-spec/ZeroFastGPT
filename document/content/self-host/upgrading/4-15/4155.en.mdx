---
title: 'V4.15.5'
description: 'gptGO V4.15.5 Release Notes'
...
- Update the `fastgpt-app` (gptGO core service) image tag to `v4.15.5`.
- Update the `fastgpt-pro` (gptGO commercial edition) image tag to `v4.15.5`.
...
2. Added the SoMark PDF enhanced parsing provider. Configure it with `SOMARK_API_KEY`; when multiple PDF providers are configured, gptGO uses this priority order: custom PDF parsing service, SoMark, TextIn, then Doc2x. See the [environment variable configuration](../../config/env) for details.
