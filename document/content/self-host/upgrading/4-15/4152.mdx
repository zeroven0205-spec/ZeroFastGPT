---
title: 'V4.15.2'
description: 'gptGO V4.15.2 更新说明'
...
旧值不再兼容。继续使用 `default` 或 `pi` 会导致环境变量校验失败，gptGO 无法启动。未配置 `AGENT_ENGINE` 时，可正常启动，系统默认使用 `fastAgent`。

### 3. 修改文件下载模式变量

V4.15.2 新增 `STORAGE_DOWNLOAD_URL_MODE` 环境变量，默认值为 `short-proxy`。

- `short-proxy`：返回 gptGO 短链，由 gptGO App 代理文件下载。
- `short-redirect`：返回 gptGO 短链，校验后跳转到临时 S3/CDN 地址。

如需使用短链但不希望文件流量经过 gptGO App，可配置：
...
- 更新 gptGO-app(gptGO 主服务) 镜像 tag: v4.15.2
- 更新 gptGO-pro(gptGO 商业版) 镜像 tag: v4.15.2
- 更新 gptGO-plugin 镜像 tag: v1.0.2
