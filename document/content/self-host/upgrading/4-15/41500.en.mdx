---
title: 'V4.15.0'
description: 'gptGO V4.15.0 Release Notes'
...
# Maximum Dataset file parsing queue concurrency
DATASET_PARSE_MAX_PROCESS=10
# Maximum vector indexing queue concurrency
...
3. Change the database name in `fastgpt-plugin`'s `MONGODB_URI` so it does not conflict with gptGO's MongoDB database name. Example: `mongodb://myusername:mypassword@fastgpt-mongo:27017/fastgpt-plugin?authSource=admin`.
...
See the [4.15 deployment YAML](https://github.com/zeroven0205-spec/) for the complete OpenSandbox setup. The 4.15 Docker Compose deployment file already includes OpenSandbox Server, Volume Manager, Agent Sandbox Proxy, and the image pre-pull services.
...
- Update gptGO-app (gptGO main service) image tag: v4.15.0
- Update gptGO-pro (gptGO commercial edition) image tag: v4.15.0
- Update gptGO-code-sandbox image tag: v4.15.0
- Update gptGO-plugin image tag: v1.0.0
- Update aiproxy image tag: v0.6.5

If `opensandbox` is enabled, also update:

- gptGO-agent-sandbox-proxy image tag: v0.2.0
- gptGO-agent-sandbox image tag: v0.2.0
...
1. Download the [zip package](https://github.com/zeroven0205-spec/) that contains all system tools.
...
You can also install them one by one from the plugin marketplace: [https://github.com/zeroven0205-spec/](https://github.com/zeroven0205-spec/). The environment variable default now points to this address, so no marketplace-related variables are required.
...
3. Prepare a reachable gptGO `{{host}}` and `{{rootkey}}`. All APIs below require `rootkey`.
...
1. API Key behavior has changed. gptGO no longer distinguishes between app keys and system keys; only system keys are kept. For OpenAI SDK compatibility, pass the token as `apikey-appId`. Existing API keys remain compatible and continue to work. For details, see the [gptGO API documentation](../../../openapi/intro).
...
1. Added Skills. Agent V2 can bind and run static Skills.
...
5. The commercial edition now supports local direct-connect debugging for gptGO plugins.
...
9. Dataset search now supports native multimodal embedding models, image-to-image search, and permission filtering in Agent mode.
...
14. Added DingTalk Dataset integration for third-party Datasets.
...
13. Improved the Dataset search test interaction and Dataset data editing modal.
14. When a Dataset is deleted, app orchestration now shows a graceful prompt.
15. Improved error prompts during Dataset indexing and added one-click retry for all failed items.
16. Invalid Dataset citation markers are now filtered out.
...
20. Strengthened security protection for third-party Dataset requests, HTTP tool parsing, IP detection, and Code Sandbox AST checks.
...
27. Improved long-name display for apps, Datasets, files, and folders: names are truncated when they exceed the available width, and the full name is shown on hover.
...
31. Added virtual list rendering for apps and Datasets to improve large-list performance.
32. LLM request traces now use team-isolated queries to prevent request IDs from exposing request bodies, retrieved Dataset chunks, and model responses across teams.
...
4. App-related APIs now use Zod schemas consistently and generate documentation.
