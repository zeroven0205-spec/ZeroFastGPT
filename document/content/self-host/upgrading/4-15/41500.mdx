---
title: 'V4.15.0'
description: 'gptGO V4.15.0 更新说明'
...
OpenSandbox 的完整配置请参考 [4.15 部署 yml](https://github.com/zeroven0205-spec/)。4.15 的 Docker Compose 部署文件已经内置 OpenSandbox Server、Volume Manager、Agent Sandbox Proxy 和预拉取镜像服务。
...
- 更新 gptGO-app(gptGO 主服务) 镜像 tag: v4.15.0
- 更新 gptGO-pro(gptGO 商业版) 镜像 tag: v4.15.0
- 更新 gptGO-code-sandbox 镜像 tag: v4.15.0
- 更新 gptGO-plugin 镜像 tag: v1.0.0
- 更新 aiproxy 镜像 tag: v0.6.5

如果启用 `opensandbox`，需同步更新下面镜像:

更新 gptGO-agent-sandbox-proxy 镜像 tag: v0.2.0
更新 gptGO-agent-sandbox 镜像 tag: v0.2.0
...
1. 下载所有系统工具的 [zip 包](https://github.com/zeroven0205-spec/)。
2. 打开 `fastgpt` 网页 - 点击 `管理员` navbar - 点击添加插件 - 点击 `导入/更新插件` - 上传 zip - 确认。

也可以打开插件市场逐个下载安装，插件市场地址为: [https://github.com/zeroven0205-spec/](https://github.com/zeroven0205-spec/)，环境变量默认值已变成该地址，不设置相关环境变量即可。
...
3. 准备可访问 gptGO 的 `{{host}}` 和 `{{rootkey}}`。下面所有接口都需要 `rootkey`。
...
如果使用过旧版 agent sandbox workspace，可通过该接口修正历史 agent sandbox 状态字段，并按需把不活跃 workspace 归档到 S3。该步骤不影响新生成的 agent sandbox；不执行也不影响 v4.15 正式版升级，只是不会自动归档旧 workspace。可以手动删除旧的沙盒。
...
1. ApiKey 功能调整，不再区分应用 key 和系统 key，只保留系统 key，如需兼容 openai sdk 用法，可使用 `apikey-appId` 的方式传递 Token。已有的 apikey 保持兼容，不影响使用。具体和查阅 [gptGO API 文档说明](../../../openapi/intro)
