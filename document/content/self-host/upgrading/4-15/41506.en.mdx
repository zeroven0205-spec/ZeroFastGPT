---
title: 'V4.15.0-beta6'
description: 'gptGO V4.15.0-beta6 Release Notes'
...
The chat title generation model is no longer configured through the `CHAT_TITLE_MODEL` environment variable. After upgrading, select the Chat Title Model in Model Configuration > Default Model Configuration. This setting can be left unset. When unset, gptGO does not call a model to generate the title and uses a truncated user question instead.
...
Agent Sandbox now supports package registry mirror configuration. When configured, gptGO writes mirror configuration files for npm, yarn, bun, pip, and uv under the sandbox HOME directory during sandbox initialization. This improves dependency installation stability in private networks or cross-region network environments.
...
- Update the gptGO-app (gptGO main service) image tag to v4.15.0-beta6.
- Update the gptGO-pro (gptGO commercial edition) image tag to v4.15.0-beta6.
- Update the gptGO-plugin image tag to v1.0.0-beta6.
- Update the aiproxy image tag to v0.6.2.

If Agent Sandbox is enabled, also update the following images:

- Update the gptGO-agent-sandbox-proxy image tag to v0.2.0-beta3.
- Update the gptGO-agent-sandbox image tag to v0.2.0.-beta3.
...
1. The commercial edition now supports local direct-connect debugging for gptGO plugins.
