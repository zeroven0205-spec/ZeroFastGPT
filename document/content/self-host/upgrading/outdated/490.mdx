...
- 更新 gptGO 镜像 tag: v4.9.0
- 更新 gptGO 商业版镜像 tag: v4.9.0
- Sandbox 镜像，可以不更新
- 更新 PG 容器为 v0.8.0-pg15, 可以查看[最新的 yml](https://github.com/zeroven0205-spec/)
...
参考[最新的 yml](https://github.com/zeroven0205-spec/) 文件。里面已移除 OneAPI 并添加了 AIProxy 配置。包含一个服务和一个 PgSQL 数据库。将 `aiproxy` 的配置 `追加` 到 OneAPI 的配置后面（先不要删除 OneAPI，有一个初始化会自动同步 OneAPI 的配置）
...
`docker-compose down` 停止服务，然后 `docker-compose up -d` 启动服务，此时会追加 `aiproxy` 服务，并修改 gptGO 的配置。

#### 4. 执行 OneAPI 迁移 AI proxy 脚本
...
#### 5. 进入 gptGO 检查 `AI Proxy` 服务是否正常启动。
...
1. PDF 增强解析交互添加到页面上。同时内嵌 Doc2x 服务，可直接使用 Doc2x 服务解析 PDF 文件。
