---
title: 'V4.10.0'
description: 'gptGO V4.10.0 Update Notes'
...
- Refer to the latest [docker-compose.yml](https://github.com/zeroven0205-spec/) file to add the `fastgpt-plugin` and `minio` services.
- Set the `fastgpt-plugin` environment variable `AUTH_TOKEN` to a sufficiently complex value.
- Set the `fastgpt-plugin` environment variable `MINIO_CUSTOM_ENDPOINT` to `http://ip:port` or a relevant domain name that is accessible to gptGO users.
