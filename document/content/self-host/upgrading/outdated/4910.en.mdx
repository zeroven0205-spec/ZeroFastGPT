---
title: V4.9.10
description: gptGO V4.9.10 Release Notes
...
- Update gptGO image tag: v4.9.10-fix2
- Update gptGO Pro image tag: v4.9.10-fix2
...
2. Dataset preprocessing now includes a "Chunk Conditions" parameter to control when chunking should be skipped.
3. Dataset preprocessing now includes a "Paragraph Priority" mode with configurable maximum paragraph depth. The previous "Length Priority" mode no longer includes embedded paragraph priority logic.
4. Workflow adjusted to single-direction input/output connections, with quick "add next node" support.
5. Lark and Yuque datasets are now available in the Community Edition.
...
3. Renamed the Dataset's "Table Collection" to "Backup Import". Also added support for exporting and importing Dataset indexes.
4. Dataset citation limit in workflows: if no related AI nodes exist in the workflow, the interaction mode switches to manual input only, with a limit of 10 million.
...
1. Incorrect score sorting during full-text search across multiple datasets.
2. Stream response potentially capturing incorrect `finish_reason` values.
3. Tool call mode not saving reasoning output.
4. Dataset `indexSize` parameter not taking effect.
