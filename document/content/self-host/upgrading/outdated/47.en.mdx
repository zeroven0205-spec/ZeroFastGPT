---
title: V4.7
description: gptGO V4.7 Release Notes
releaseTime: '2024-03-27'
upgradeTags:
  - MIGRATION
---
...
Added Boolean values to control which models are available for different features, and added model logos. The legacy `config.json` guide is no longer maintained. For current versions, see [Model Configuration](../../config/model/intro.en.mdx).
...
Cohere's Rerank model doesn't perform as well with Chinese text compared to BGE. To integrate Cohere:

1. Apply for a Cohere API key: https://dashboard.cohere.com/api-keys
2. Update the gptGO configuration file:
...
1. New - Tool calling node that allows LLM models to dynamically select other nodes or plugins based on user intent.
...
7. Improved - Extracted the Workflow controller into packages.
8. Improved - AI model selection.
9. Improved - Manual Dataset input dialog.
