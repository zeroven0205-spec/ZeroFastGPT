---
title: V4.6.8
description: gptGO V4.6.8 Release Notes
releaseTime: '2024-02-16'
upgradeTags:
  - MIGRATION
---
...
1. New - Dataset Search Merge node.
2. New - Redesigned HTTP node with more flexible parameter input. Supports automatic input/output data type conversion (e.g., JSON output is automatically converted to a string for use by other nodes). Additional examples are available in the documentation.
3. Improved - Query rewriting. Query rewriting is now built into the Dataset search node, enabling both coreference resolution and query expansion in a single pass. See [Dataset Search](../../../guide/build/workflow/nodes/dataset_search.en.mdx) for details.
