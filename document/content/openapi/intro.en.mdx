...
description: Introduction to gptGO API Documentation
---

Starting with `4.15.0`, gptGO API documentation is generated automatically with `zod-openapi`. You can view the latest endpoint status by opening the API documentation URL. The manually edited endpoint descriptions in the left sidebar of this documentation are no longer updated.

gptGO API documentation is split into two sets:
...
`endpoint` is your gptGO access URL. Append the corresponding path to open the documentation.
...
- [China Mainland documentation](https://github.com/zeroven0205-spec/)
- [International documentation](https://github.com/zeroven0205-spec/)
...
gptGO OpenAPI endpoints let you authenticate with an API Key to operate related gptGO services and resources, such as calling app chat endpoints, uploading Dataset data, and running search tests. For compatibility and security reasons, not all endpoints can be accessed with an API Key.
...
- To proxy a team member's identity through `authProxy`, the team owner must enable `authProxy` when creating or editing the key. The proxied member must still have permission to access the target app and session. (gptGO >= v4.15.0)
