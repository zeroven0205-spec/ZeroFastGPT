---
title: Chat API
sidebarTag: No longer maintained
description: gptGO OpenAPI Chat Interface
---

# How to Get AppId

You can find the AppId in your app details URL.
...
- For OpenAI SDK compatibility, `Authorization: Bearer <apiKey>-<appId>` is also supported. The suffix is only a transport compatibility format and is not stored.
...
{/* Chat now provides both v1 and v2 endpoints. Choose the version you need. V2 was added in 4.9.4, and v1 is no longer maintained. */}

## Start Chat

The `v1` chat API is compatible with the `GPT` interface! If you're using the standard `GPT` official API, you can access gptGO by simply changing the `BaseUrl` and `Authorization` . However, note these rules:
...
- chatId: string | undefined.
  - Empty or omitted: gptGO context is not used, and context is built entirely from `messages` .
  - Non-empty string: uses `chatId` for the chat, automatically reads messages from the gptGO session, and uses only the last item in `messages` as the user question. Other messages are ignored. Make sure `chatId` is unique and shorter than 250 characters.
- messages: Same structure as [GPT chat messages](https://platform.openai.com/docs/api-reference/chat/object) .
- responseChatItemId: string | undefined. If provided, gptGO uses it as the response message ID and stores it in the database. Make sure it is unique under the current `chatId` .
...
- variables: Node variables. This object replaces `[key]` placeholders in input fields.
...
{
  "responseData": [
    // Response values vary by node and version. Log the response to inspect the latest structure.
    {
...
data: {"status":"running","name":"Dataset search"}

event: flowNodeStatus
data: {"status":"running","name":"AI Chat"}
...
- When calling a Plugin-type App, the endpoint uses `detail` mode by default.
...
event: flowResponses
data: [{"nodeId":"fdDgXQ6SYn8v","moduleName":"AI Chat","moduleType":"chatNode","totalPoints":0.033,"model":"FastAI-3.5","tokens":33,"query":"你好","maxToken":2000,"historyPreview":[{"obj":"Human","value":"你好"},{"obj":"AI","value":"你好！有什么可以帮助你的吗？"}],"contextTotalLen":2,"runningTime":1.42},{"nodeId":"pluginOutput","moduleName":"Plugin output","moduleType":"pluginOutput","totalPoints":0,"pluginOutput":{"result":"你好！有什么可以帮助你的吗？"},"runningTime":0}]
```
...
Event values:
    ...
    - answer: Text returned to the client and counted as the final answer
    - fastAnswer: Specified Reply text returned to the client and counted as the final answer
    - toolCall: Run a tool
    - toolParams: Tool parameters
    - toolResponse: Tool response
    - flowNodeStatus: Status of the current node
    - flowResponses: Complete node responses
    - updateVariables: Updated variables
    - error: Error
...
- chatId - The ID of a session under an app
...
| questionGuide | object |          | Custom configuration. If omitted, uses the latest published configuration for the App ID. |
