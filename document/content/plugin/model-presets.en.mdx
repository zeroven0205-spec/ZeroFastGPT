...
description: Model preset notes for the gptGO plugin system
---

Model presets are maintained in the `fastgpt-plugin` repository. They provide gptGO with built-in model providers, model lists, model capabilities, and default request parameters. After gptGO loads these static presets, users can select the corresponding models in model configuration, AIProxy channels, and plugin-related features.
...
| `quoteMaxToken`      | Maximum token budget gptGO can use for cited Dataset content                   |
...
- Some default LLM capability switches, such as Dataset processing, classification, extraction, tool calling, and evaluation.
...
- AIProxy protocol: decides whether the protocol appears in the AIProxy channel list. AIProxy routes requests to the corresponding adaptor by `channelId`, and gptGO uses `channel-avatar/{avatar}.svg` as the channel avatar.

If you are only adding model presets, you may not need to add an AIProxy protocol. Maintain `aiproxyChannels` only when gptGO needs to display that protocol in the AIProxy channel list.
...
| AIProxy type              | ID   | gptGO `channelId` |
| ------------------------- | ---- | ----------------- |
...
### 2. Add the protocol declaration in gptGO-plugin
...
| `name`      | Multilingual display name in the gptGO channel list                    |
