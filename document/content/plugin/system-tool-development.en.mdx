...
description: gptGO system tool development guide
---

## Introduction

This document targets system tool development after gptGO v4.15.0. The new gptGO Plugin service unifies system tools, model presets, and similar capabilities as installable, updatable, runtime-isolated plugin packages. A plugin is eventually delivered to the gptGO Plugin service as a `.pkg` file.

The currently stable system tool plugin types are:

- Single tool: one plugin exposes one tool and is declared with `defineTool()`.
- Toolkit: one plugin exposes multiple related child tools and is declared with `defineToolSet()`.

System tool plugins run in the runtime provided by the gptGO Plugin service. The gptGO main service invokes tools through the plugin service, and plugin code uses `@fastgpt-plugin/sdk-factory` to describe input, output, secret configuration, and execution logic.

## Differences From The Legacy Mechanism

1. The deployment relationship between gptGO and gptGO Plugin remains an external extension model, and the overall architecture is still microservice-based.
...
4. Plugin metadata, input/output schemas, secret schemas, and icon assets are included in build artifacts for use by gptGO pages, workflows, and Agents.
...
https://github.com/zeroven0205-spec/
...
## 4. Implement a Toolkit

Use `defineToolSet()` for Toolkits. Put shared information in the top-level `manifest` and `secretSchema`, and declare each child tool's independent `id`, name, description, and handler in `children`.
...
| Toolkit child icon | `<childId>.logo.svg`, `<childId>.logo.png`, and similar names               |
...
Remote debugging connects a locally developed plugin to a gptGO test environment. The gptGO page authenticates the user and generates a debug link, while the CLI uses that link to create a WSS debug channel. Debug plugins are visible only to the current debugger.

Before using it, confirm that the test environment has deployed the gptGO Plugin service and Connection Gateway, and that your local machine can reach the Gateway WSS endpoint returned by the test environment.

### 7.1 Generate A Debug Link

1. Sign in to the gptGO test environment.
...
After startup, paste the debug link copied from gptGO into the TUI. The CLI exchanges the connection key from the link for a short-lived WSS connect token, then mounts local plugins to the gptGO debug channel.

Scripts and Agents can use non-interactive mode:

```bash
fastgpt-plugin dev --no-interactive \
  --connect "https://gptgo.example.com/api/plugin/debug-channel/connection-key/exchange?connectionKey=fpg_dbg_..."
```

When passing only a raw connection key, tell the CLI where the exchange endpoint is:

```bash
FASTGPT_PLUGIN_DEBUG_CONNECT_URL=https://gptgo.example.com/api/plugin/debug-channel/connection-key/exchange \
...
After the CLI reports that remote debugging is ready, return to the gptGO test environment:
...
The End Debugging action in gptGO revokes the current member's debug channel and removes the debug plugin entry from the page. If the debug link is exposed, the signed-in member changes, or authorization needs to be renewed, use Refresh Link to generate a new link.
...
- Toolkit `children[].id` values are stable and unique.
...
If you do not need official inclusion, see [Upload System Tool](../guide/build/tools/system-plugins/upload_system_tool.en.mdx) to use the plugin in your own gptGO deployment.
...
- [gptGO Plugin Repository](https://github.com/zeroven0205-spec/)
- [System Plugin Development Guide](https://github.com/zeroven0205-spec/)
- [SDK Factory Guide](https://github.com/zeroven0205-spec/)
- [CLI Guide](https://github.com/zeroven0205-spec/)
