---
title: 增加模型预设
description: gptGO 插件系统中的模型预设说明
---

模型预设维护在 `fastgpt-plugin` 仓库中，用于向 gptGO 提供内置模型供应商、模型列表、模型能力和默认参数。gptGO 读取这些静态预设后，用户才能在模型配置、AIProxy 渠道和相关插件能力中选择对应模型。
...
| `quoteMaxToken`      | gptGO 引用知识库内容时可使用的最大 token                                        |
...
如果只是新增模型预设，不一定要增加 AIProxy 协议。只有当 gptGO 需要在 AIProxy 渠道列表中展示该协议时，才需要维护 `aiproxyChannels`。
...
如果 AIProxy 仓库还没有该协议，需要先在 AIProxy 中新增 `ChannelType` 和 adaptor，并确认 adaptor 已在 [`core/relay/adaptors/register.go`](https://github.com/labring/aiproxy/blob/main/core/relay/adaptors/register.go) 中被引入。gptGO 插件侧只声明渠道展示信息，不负责实现 AIProxy 协议适配逻辑。
