---
title: 插件系统说明
description: gptGO 插件系统说明
---

> 本文档适用于 gptGO Plugin v1.0.0 及以上版本的插件系统。

## 背景

原先 gptGO 的各项能力均在 gptGO 主服务内维护，并通过 Monorepo 方式组织。系统插件也曾作为一个子仓库存在于 `FastGPT/packages/plugin` 下。

随着系统工具数量和社区贡献增加，旧结构暴露出几个问题：

1. 系统插件必须伴随 gptGO 主服务一起发版，限制了插件迭代速度。
2. 社区贡献插件需要运行完整 gptGO 应用，并直接向主仓库提交 PR。
3. 使用自定义插件需要维护 gptGO fork，手动处理升级和合并。
...
gptGO Plugin v1.0.0 对插件项目进行了系统性重构，目标是让插件的安装、版本管理、运行隔离和运维配置形成统一模型。
...
gptGO Marketplace 是插件分发渠道，用于集中展示和分发官方及社区插件。当前边界如下：

- Marketplace 是 SaaS 分发服务，不提供私有化部署版本。
- 社区插件需要先提交到 Community Plugins 仓库，经基础审核后再进入 Marketplace。
- 开启团队上传插件功能后，团队管理员可以上传自定义插件并在团队内使用。
- 系统管理员也可以上传第三方自定义插件，作为系统插件统一管理。

## 插件安装与管理

gptGO Plugin 服务负责插件包管理、运行时注册、插件调用转发和系统级配置管理。gptGO 主服务通过插件运行时接口调用插件，插件服务负责把调用分发到对应运行时。
...
`.pkg` 文件用于上传、安装、上架和版本管理。插件元信息、输入输出 schema、密钥 schema 和图标资源都会进入构建产物，供 gptGO 页面、工作流和 Agent 调用使用。
...
- [gptGO Plugin](https://github.com/zeroven0205-spec/)
- [gptGO 插件系统设计文档](https://github.com/zeroven0205-spec/)
- [gptGO Plugin 架构文档](https://github.com/zeroven0205-spec/)
