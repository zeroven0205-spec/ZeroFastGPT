...
description: gptGO Batch Processing node overview and usage
---

## Node Overview

The **Batch Processing** node was introduced in gptGO V4.8.11. It allows workflows to iterate over array-type input data, processing one element at a time and automatically executing subsequent nodes until the entire array is processed.
...
The **Batch Processing** node extends workflow capabilities through automation, enabling gptGO to handle batch tasks and complex data processing pipelines. It significantly improves efficiency when processing large-scale data or scenarios requiring multiple iterations.
...
2. Configure the Batch Processing node
   ...
   - Add a Search Glossary node to look up proper nouns from a terminology Dataset before translation.
