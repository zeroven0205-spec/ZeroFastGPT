---
title: File Input
description: gptGO file input feature overview
---

Starting from version 4.8.9, gptGO supports configuring file uploads in both `Basic Mode` and `Workflows`. This guide covers how to use file input and explains the difference between document parsing and multimodal file handling.
...
You can also use Workflows to extract or analyze document content, then pass the results to HTTP requests or other nodes to build a document processing pipeline.
...
In gptGO's chat history, messages with role=user store their value in this structure:
...
2. Multimodal recognition: handles media files such as images, audio, and video. gptGO converts them into model-readable input, and a model with the corresponding capability reads them.
...
AI nodes (AI Chat / Tool Calling) have a document URL input that lets you cite documents by URL.
...
3. Tool Calling: supports direct document citation selection — no need to attach a document parsing tool. Automatically parses files from chat history.
4. AI Chat: supports direct document citation selection — no need to go through the document parsing node. Automatically parses files from chat history.
