...
description: gptGO WeChat Official Account Integration Tutorial
---

Starting from version 4.8.10, gptGO commercial edition supports direct WeChat Official Account integration without additional APIs.

**Note: Currently only verified official accounts are supported (both Service Accounts and Subscription Accounts).**

## 1. Create a Publishing Channel in gptGO

In gptGO, select the app you want to integrate. On the _Publishing Channels_ page, create a new WeChat Official Account publishing channel and fill in the basic information.
...
### 2. Enter the 3 parameters into the gptGO configuration dialog.

![Image](/imgs/offiaccount-3.png)

## 3. Add gptGO IP to IP Whitelist

![Image](/imgs/offiaccount-4.png)

Self-hosted users can check their own IP address.

International edition users (cloud.gptGO.io) can add the following IP whitelist:
...
China Mainland users (gptGO.cn) can add the following IP whitelist:
...
1. Randomly generate an AES Key and enter it into the gptGO configuration dialog.

2. Select the encryption mode as Secure Mode.

## 5. Get URL

1. Confirm creation in gptGO and get the URL.
...
Now when users send messages to the official account, messages will be forwarded to gptGO, and conversation results will be returned through the official account.
