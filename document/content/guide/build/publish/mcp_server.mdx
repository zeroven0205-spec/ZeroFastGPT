---
title: MCP 发布
description: 快速了解 gptGO MCP server
---
...
gptGO MCP Server 功能允许你选择 `多个` 在 gptGO 上构建好的应用，以 MCP 协议对外提供调用 gptGO 应用的能力。

gptGO 支持 `Streamable HTTP` 协议；私有化部署还可以通过独立的 MCP Server 服务提供兼容的 `SSE` 协议。

## gptGO 使用 MCP server

### 1. 创建 MCP server

登录 gptGO 后，打开 `工作台`，点击 `MCP server`，即可进入管理页面，这里可以看到你创建的所有 MCP server，以及他们管理的应用数量。
...
可以在支持 MCP 协议的客户端使用这些地址，来调用 gptGO 应用，例如：`Cursor`、`Cherry Studio`。下面以 Cursor 为例，介绍如何使用 MCP server。
...
发送一个关于 `gptGO` 的问题后，可以看到，cursor 调用了一个 MCP 工具（描述为：查询 gptGO 知识库），也就是调用 gptGO 应用去进行处理该问题，并返回了结果。
...
| `x-fastgpt-auth-proxy-tmb-id`   | gptGO 团队成员 ID    | 适合已经保存团队成员 ID 的系统 |
...
工具列表只公开工具名称和参数等元数据；gptGO 会在每次工具执行时重新校验成员状态和应用权限。

## 私有化部署 MCP server 问题

私有化部署版本的 gptGO，需要升级到 `v4.9.6` 及以上版本才可使用 MCP server 功能。
...
在 gptGO 容器中配置 `SSE_MCP_SERVER_PROXY_ENDPOINT`，值为客户端可访问的 `fastgpt-mcp-server` 地址，末尾不要携带 `/`，例如:
...
修改环境变量后，需要重启 gptGO 服务。启动后，可以在工作台看到 MCP server 服务选项。
