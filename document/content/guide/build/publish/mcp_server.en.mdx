---
title: MCP Server
description: A quick overview of gptGO MCP Server
---
...
gptGO's MCP Server feature lets you select `multiple` apps built on gptGO and expose them via MCP protocol for external consumption.

gptGO supports the `Streamable HTTP` transport. Self-hosted deployments can also expose the compatible `SSE` transport through the standalone MCP Server service.

## Using MCP Server in gptGO

### 1. Create an MCP Server

After logging into gptGO, open `Studio` and click `MCP Server` to access the management page. Here you can see all your MCP Servers and the number of apps each one manages.
...
Use the URL in any MCP-compatible client to call your gptGO apps — for example, `Cursor` or `Cherry Studio`. Here's how to set it up in Cursor.
...
After sending a question about `gptGO`, you'll see Cursor invoke an MCP tool (described as: query gptGO Dataset), which calls the gptGO app to process the question and return results.
...
Identity proxy lets a caller select the team member who executes an MCP tool. gptGO checks that member's read permission for the target application and attributes chat and runtime records to that member. Use it when a gateway or shared MCP client sends requests on behalf of different team members.
...
Pass the identity in the MCP transport headers, not in the tool arguments. gptGO accepts these headers:
...
| `x-fastgpt-auth-proxy-tmb-id`   | gptGO team member ID       | Use this when your system already stores gptGO team member IDs |
...
```json
{
  "mcpServers": {
    "fastgpt": {
      "url": "https://gptgo.example.com/api/mcp/app/<MCP_KEY>/mcp",
      "headers": {
...
The tool list exposes only metadata such as tool names and parameters. gptGO rechecks team membership and application permission for every tool call.

## Self-Hosted MCP Server Setup

Self-hosted gptGO deployments require version `v4.9.6` or higher to use MCP Server.
...
### Update gptGO Container Environment Variables

Configure `SSE_MCP_SERVER_PROXY_ENDPOINT` in the gptGO container. Set it to the client-accessible `fastgpt-mcp-server` URL without a trailing `/`. For example:

```yaml
environment:
  SSE_MCP_SERVER_PROXY_ENDPOINT: https://mcp.example.com
```

### Restart gptGO

Restart gptGO after changing the environment variable:
