...
分享链接身份验证设计的目的在于，将 gptGO 的对话框快速、安全的接入到你现有的系统中，仅需 2 个接口即可实现。该功能目前只在商业版中提供。
...
`gptGO` 将会判断 `success` 是否为 `true` 决定是允许用户继续操作。`message` 与 `msg` 是等同的，你可以选择返回其中一个，当 `success` 不为 `true` 时，将会提示这个错误。
...
原始的链接：`https://example.com/chat/share?shareId=648aaf5ae121349a16d62192`

完整链接: `https://example.com/chat/share?shareId=648aaf5ae121349a16d62192&authToken=userid12345`

这个 `authToken` 通常是你系统生成的用户唯一凭证（Token 之类的）。gptGO 会在鉴权接口的 `body` 中携带 token=[authToken] 的参数。
...
源分享链接：`https://example.com/chat/share?shareId=64be36376a438af0311e599c`

修改后：`https://example.com/chat/share?shareId=64be36376a438af0311e599c&authToken=demo-token`
