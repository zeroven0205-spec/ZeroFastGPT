---
title: Web Site Sync
description: Introduction and usage of the gptGO Web Site Sync feature
---
...
Web Site Sync uses crawler technology to automatically discover all pages under the `same domain` from an entry URL, supporting up to `200` sub-pages. For compliance and security reasons, gptGO only supports crawling `static sites`, primarily intended for quickly building datasets from documentation sites.
...
```bash
curl https://github.com/zeroven0205-spec/
```

## How to Use

### 1. Create a New Dataset and Select Web Site Sync
...
## Create an App and Bind the Dataset
...
In the earlier demo, we used multiple selectors for the gptGO documentation site, separated by commas.
