...
description: Introduction and usage of the gptGO API File Library
---
...
gptGO supports local file imports, but in many cases users already have an existing document library. Re-importing files would create duplicate storage and complicate management. To address this, gptGO offers an API File Library that connects to your existing document library through simple API endpoints, with flexible import options.

The API File Library lets you integrate your existing document library seamlessly. Implement a few endpoints that conform to gptGO's API File Library specification, provide the service's baseURL and token when creating a Dataset, and you can browse and selectively import files directly from the UI.

## How to Use the API File Library

When creating a Dataset, select the API File Library type and configure the key parameters: the baseURL of your file service and the request header for authentication. As long as your endpoints conform to gptGO's specification, the system will automatically fetch and display the complete file list for selective import.
...
<Alert icon=" " context="success">
  - parentId - Parent ID, optional. If not provided or null, the configured basePath will be used as
  the root directory - searchKey - Search keyword, optional
</Alert>
...
<Alert icon=" " context="success">
  - id - File ID - name - File name - parentId - Parent ID, null indicates root directory - type -
  File type, file or folder - updateTime - Update time - createTime - Creation time
</Alert>
