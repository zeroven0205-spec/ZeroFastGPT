---
title: DingTalk Dataset
description: How to connect DingTalk Dataset to gptGO
---

gptGO supports connecting DingTalk Dataset through a DingTalk internal enterprise app. When creating the dataset, enter `App Key`, `App Secret`, and `User ID`. After creation, open the dataset detail page, click `Add file`, and select the DingTalk workspace, online documents, or folders to import.
...
![Create a DingTalk app](/imgs/image-dd3.png)
...
## 2. Get the gptGO fields

![Get App Key and App Secret](/imgs/image-dd4.png)

| gptGO field  | Where to get it in DingTalk                                                                                                                                                                                                                |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
- We recommend using a dedicated DingTalk member as the gptGO sync account and granting it read-only access to the target workspace.
- Workspaces that this member cannot access will not appear in gptGO.

## 3. Enable DingTalk app permissions

![Enable DingTalk app permissions](/imgs/image-dd5.png)
...
## 4. Create a DingTalk dataset in gptGO

1. Open the gptGO dataset list and click `New`.
2. Select `DingTalk Dataset` under external document sources.
3. Enter:
   - `App Key`
   - `App Secret`
   - `User ID`
...
When a folder is selected, gptGO recursively imports supported online documents under that folder.

When DingTalk document content changes, click `Sync` from the imported file menu. gptGO will read the latest content and update indexes.
