...
description: How to integrate a third-party Dataset with gptGO
sidebarTag: DEV
---

import { Alert } from '@/components/docs/Alert';

There are many document libraries available online, such as Lark, Yuque, and others. Different gptGO users may use different document libraries. gptGO has built-in support for Lark and Yuque, but if you need to integrate other document libraries, follow this guide.

## Unified API Specification

To provide a unified interface for different document libraries, gptGO defines a standard API specification with 4 endpoints. See the [API File Library endpoints](./api_dataset.en.mdx).
...
First, go to `FastGPT\packages\global\core\dataset\apiDataset.d.ts` in the gptGO project and add the third-party document library server type. Design the fields based on your needs. For example, the Yuque Dataset requires `userId` and `token` for authentication.
