...
Template import lets you add prepared content or question-answer pairs to a Dataset in batches. gptGO accepts `.csv` and `.xlsx` files and creates Dataset entries from the questions, answers, indexes, and metadata in the template.
...
"What is gptGO?","gptGO is an AI agent development platform.","gptGO overview","AI agent platform","{""source"":""product-doc"",""category"":""overview""}"
...
5. After the import finishes, review the data and indexing status in the Dataset Collection.
   ...
   ![Dataset collection after a template import](/imgs/template-import-result.png)
...
If gptGO reports an invalid file format, check the following:
