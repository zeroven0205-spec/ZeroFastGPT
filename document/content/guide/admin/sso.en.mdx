...
description: gptGO External Member System Integration and Configuration
---
...
To simplify integration with **external member systems**, gptGO provides a set of **standard interfaces** for connecting to external systems, along with a gptGO-SSO-Service image that serves as an **adapter**.

Through these standard interfaces, you can:

1. SSO login. After a callback from an external system, create a user in gptGO.
2. Member and organizational structure sync (referred to as "member sync" below).

**How It Works**

gptGO-pro includes a standard set of SSO and member sync interfaces. The system performs SSO and member sync operations based on these interfaces.

gptGO-SSO-Service aggregates SSO and member sync interfaces from different sources and converts them into the format recognized by gptGO-pro.
...
Set the gptGO-pro environment variable to enable automatic member sync:
...
The redirect URL should follow the format `https://your-domain.example/login/provider` — replace the domain with your publicly accessible gptGO domain.
![](/imgs/sso5.png)
...
fastgpt-sso:
  ...
  environment:
    ...
    - FEISHU_REDIRECT_URI=https://your-domain.example/login/provider
...
2.  Create an internal app for gptGO:
...
#### 2. Use the latest image to deploy the gptGO-sso relay service
...
- A member's department list is filtered to visible departments; a member who belongs only to invisible departments is not synchronized to gptGO.
...
1. Login authorization endpoint (users are redirected here with parameters after clicking the SSO button), e.g., `http://example.com/oauth/authorize`
   ...
   `https://your-domain.example/login/provider?code=4/P7qD2qAz4&state=xyz`
...
gptGO-pro must reach the SSO service and use the same token as SSO-service.AUTH_TOKEN:
...
Below is the standard interface documentation for SSO and member sync in gptGO-pro. If you need to integrate with a non-standard system, refer to this section for development.

![](/imgs/sso18.png)

gptGO provides the following standard interfaces:
...
Returns a redirect login URL. gptGO will automatically redirect to this URL. The redirect_uri is automatically appended to the URL query string.
...
1. Self-development: Build according to the standard interfaces provided by gptGO, then enter the deployed service address into fastgpt-pro.
   You can use this template repository as a starting point: [fastgpt-sso-template](https://github.com/zeroven0205-spec/)
2. Custom development by the gptGO team:
   a. Provide the system's SSO documentation, member and organization retrieval documentation, and an external test address.
   b. In gptGO-sso-service, add the corresponding provider and environment variables, and write the integration code.
