---
title: SSO & 外部成员同步
description: gptGO 外部成员系统接入设计与配置
---
...
为了方便地接入**外部成员系统**，gptGO 提供一套接入外部系统的**标准接口**，以及一个 gptGO-SSO-Service 镜像作为**适配器**。

通过这套标准接口，你可以可以实现：

1. SSO 登录。从外部系统回调后，在 gptGO 中创建一个用户。
2. 成员和组织架构同步（下面都简称成员同步）。

**原理**

gptGO-pro 中，有一套标准的 SSO 和成员同步接口，系统会根据这套接口进行 SSO 和成员同步操作。

gptGO-SSO-Service 是为了聚合不同来源的 SSO 和成员同步接口，将他们转成 gptGO-pro 可识别的接口。
...
进入开发者后台，点击企业自建应用，在开发配置的安全设置中设置重定向 URL，重定向 URL 形如 `https://your-domain.example/login/provider` 前面的域名修改为部署后公开可访问的 gptGO 的域名
![](/imgs/sso5.png)
...
#### 2. 使用 latest 镜像部署 gptGO-sso 中转服务
...
- 成员所属部门会同步过滤为应用可见部门；只属于不可见部门的成员不会同步到 gptGO。
...
gptGO-pro 需要访问 SSO-service，并使用与 SSO-service.AUTH_TOKEN 相同的 Token：
...
然后在商业版后台开启成员同步。SYNC_MEMBER_CRON 使用 UTC 时区；留空表示不执行定时同步。
...
以下是 gptGO-pro 中，SSO 和成员同步的标准接口文档，如果需要对接非标准系统，可以参考该章节进行开发。
...
返回一个重定向登录地址，gptGO 会自动重定向到该地址。redirect_uri 会自动拼接到该地址的 query 中。
...
1. 客户自己开发：按 gptGO 提供的标准接口进行开发，并将部署后的服务地址填入 fastgpt-pro
   可以参考该模版库：[fastgpt-sso-template](https://github.com/zeroven0205-spec/) 进行开发
2. 由 gptGO 团队定制开发：a. 提供系统的 SSO 文档、获取成员和组织的文档、以及外网测试地址。b. 在 fastgpt-sso-service 中，增加对应的 provider 和环境变量，并编写代码来对接。
