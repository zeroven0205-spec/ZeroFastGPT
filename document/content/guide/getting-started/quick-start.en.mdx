---
title: 'Quick Start'
description: 'Quickly experience gptGO through four use cases: Conversational Agent, Dataset, Workflow, and Agent V2'
---

This article uses four complete use cases to help you quickly understand gptGO's core app types and complete the basic setup from simple chats to complex task orchestration.

This page is suitable for first-time gptGO users, as well as pre-sales, delivery, operations, legal, and administrative roles who want to quickly experience the platform's capabilities. After completing this page, you will build the following in order:

1. Conversational Agent: Corporate email writing assistant.
2. Dataset + Conversational Agent: Civil Code Q&A assistant.
...
- A Dataset test file, such as the Civil Code, company policies, product manuals, etc.
...
When reading, focus on three things: what kind of problems each app type is suitable for, why the key configurations are written this way, and what to observe during validation. The parameters and prompts in this article are reusable starting points; for production deployment, you can replace them with your own business materials, review rules, notification channels, and data files.

## Case 1: Conversational Agent — Corporate Email Writing Assistant

### 1.1 Use Cases

Conversational Agents are suitable for lightweight Q&A, content generation, copy refinement, and standardized output. This case does not link a Dataset or rely on complex workflows; it simply uses model configuration, prompts, and an Email tool to build a corporate email writing assistant.
...
The focus of this case is not to have the AI randomly generate an email, but to consolidate the stable requirements of corporate email writing into the prompt, such as subject lines, salutations, body structure, action items, and risk reminders. For beginners, it also serves as a minimal closed loop for understanding gptGO app configuration: first define the role, then constrain the output, and finally extend execution actions through tools.

### 1.2 Configuration Steps

1. **Create a Conversational Agent**

   Click **New** in Studio, select **Conversational Agent**, and name the app `Corporate Email Writing Assistant`.

2. **Open the App Configuration Page**

   After creation, open the app configuration page. The page is usually divided into left and right columns: the left is AI configuration, and the right is Debug Preview.
...
## Case 2: Dataset + Conversational Agent — Civil Code Q&A Assistant

### 2.1 Use Cases

A Dataset is ideal for scenarios where answers must be based on provided materials, such as policy Q&A, product manual Q&A, legal article retrieval, and customer service knowledge support. Without a Dataset, the AI primarily relies on its own model capabilities to answer questions. Once a Dataset is connected, the AI first searches your materials and then organizes answers based on the search results.

In this case, we import the _Civil Code of the People's Republic of China_ into the Dataset and create a Civil Code Q&A assistant. When users ask questions in natural language, the assistant should prioritize citing the original Civil Code text to reduce fabricated responses.

Think of the Dataset as a "reference room" configured for the AI. The model itself has general knowledge but doesn't know your company policies, product details, internal processes, or specific regulatory versions. The Dataset turns these materials into searchable content, allowing the AI to look up information before organizing an answer. For legal, policy, customer service, and after-sales scenarios, this is more controllable than relying solely on the model's memory.
...
1. **Create a Dataset**
   ...
   Click **Dataset** on the left side of the homepage, then click **New** in the top right corner. Name the Dataset `Civil Code Q&A Assistant`. Keep the other default settings for this case.

2. **Create a Text Collection**

   Click **New**, then select **Text Collection** to import local documents.
...
5. **Preview Chunking Results**
   ...
   When previewing chunks, focus on three things: whether paragraphs are abnormally cut off, whether titles and body text remain within the same semantic range, and whether tables, clauses, or numbering are still readable. If the quality of these knowledge fragments is unstable, even a well-written Prompt in the app will struggle to consistently produce accurate answers.

6. **Wait for the Dataset to Be Ready**

   Once the data status changes to "Ready," apps can use the Dataset.

7. **Create and Link a Conversational Agent**

   Create a new Conversational Agent, also named `Civil Code Q&A Assistant`. After creation, link the Dataset you just created in the app configuration.

   After linking the Dataset, the app's response flow changes from simple chat to "user question → Dataset search → model summarizes answer." This is the key difference between Case 1 and Case 2: Case 1 emphasizes content generation, while Case 2 emphasizes answering based on provided materials.

8. **Configure the Q&A Prompt**

   The Civil Code Q&A assistant needs to emphasize "answer based on the Dataset" and "cite the original text." You can use the following Prompt:
...
10. **Verify Q&A Performance**
    ...
    When verifying, don't just check whether the answer "looks like a legal answer" — also check whether it actually cites Dataset content. Test with questions covered by the materials, questions outside the materials, and ambiguous questions: covered questions should cite the original text; out-of-scope questions should state that a direct confirmation is not possible; ambiguous questions should ask the user for more facts.
...
To determine whether a task is suitable for a workflow, check if it has three characteristics: "stable steps, clear rules, and repeatable execution." Content review is a typical scenario: the input is content to be published, the rules come from a compliance Dataset, and the output is usually pass, rewrite, or reject, with the option to add human confirmation in between. This improves processing efficiency while retaining risk control.

### 3.2 Preparation

First, create a "Content Compliance Rules" Dataset and upload a simple text file. You can directly use the following rule template:
...
2. **Dataset Retrieval**: Recalls relevant rules from the content compliance rules base. It is recommended to set the citation limit to 1-2 entries.
...
3. **Call the Dataset**
   ...
   Add a Dataset retrieval node in the workflow, select the previously created content compliance rules base. The input for this node uses the user input, and the output is referenced by subsequent AI nodes.
...
4. **Configure the Content Review Node**
   ...
   Add an AI Chat node and rename it to "Content Review Node." This node is responsible for outputting the classification result based on the user input and Dataset rules.
   ...
   Note: The Dataset citation must select the previously created Dataset, otherwise the AI cannot read the rule content.
...
9. **Configure the Sensitive but Rewritable Branch**
   ...
   When the review result is "Sensitive but Rewritable," add an AI Chat node to automatically rewrite the content. You can use the following prompt:
...
10. **Configure the Prohibited Branch**
    ...
    When the review result is "Prohibited," add an AI Chat node to output a rejection explanation. You can use the following prompt:
...
11. **Verify the Complete Workflow**
    ...
    When verifying the complete workflow, it is recommended to record the input, review classification, branch entered, and final output for each test case. If the output does not meet expectations, troubleshoot node by node: first check whether the Dataset recalled the correct rules, then whether the review node classified accurately, and finally whether the decision node conditions and branch output are configured correctly.

### 3.5 Business Value

- **Standardize review rules into a workflow**: Embed the judgment criteria for pre-publication content review into a Dataset and node configuration, reducing reliance on personal experience passed down verbally.
...
1. **Create an Agent V2 App**

   In Studio, click **Create App**, select `Conversational Agent V2`, and name it `Intelligent Data Analysis Agent`.
...
2. **Configure Model and Prompt**
   ...
   - Label uncertain conclusions with confidence levels
...
4. **Enable Virtual Machine Capability**
   ...
   The virtual machine isolates the code execution environment. The Agent can run data analysis scripts, read uploaded files, and generate statistical results within it without directly affecting the local host environment. For use cases that require running Python, processing Excel, drawing charts, or performing batch calculations, this is an important feature that distinguishes Agent V2 from standard chat apps.
...
After completing the four cases, you can understand gptGO's common app types as a progression from simple to complex: Conversational Agent solves "how to answer and generate content," Dataset solves "what materials to base answers on," Workflow solves "what fixed process to follow," and Agent V2 solves "how to autonomously plan and execute open-ended tasks."
...
2. If answers must be based on existing materials, choose Dataset + Conversational Agent.
...
In real projects, you can also combine these capabilities. For example, a customer service assistant can use a Dataset to answer product questions and then query orders via tools; content review can use a Workflow to fix the review path while maintaining rules with a Dataset; data analysis scenarios can first use Agent V2 for exploration, then solidify stable analysis steps into a Workflow.
