...
export const GetConfigResponseSchema = z.object({
  fastgpt: z.any().optional().meta({ description: '系统 gptGO 配置' }),
  fastgptPro: z.any().optional().meta({ description: '系统 gptGO Pro 商业版配置（不含 license）' })
});
 ...
 * Method: POST
 * Description: 校验、归一化并更新 gptGO 和 gptGO Pro 的系统配置
...
export const UpdateConfigBodySchema = z.object({
  fastgpt: z
    ...
    .meta({ example: { feConfigs: {}, systemEnv: {} }, description: 'gptGO 系统配置对象' }),
  fastgptPro: z.looseObject({}).meta({ example: {}, description: 'gptGO Pro 商业版配置对象' })
});
